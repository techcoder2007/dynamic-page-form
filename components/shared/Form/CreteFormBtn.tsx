...
import { createDynamicForm } from "@/actions/form";
...
function CreateFormBtn() {
    ...
    async function onSubmit(values: formSchemaType) {
        try {
            const formId = createDynamicForm(values);
            toast({
    ...
    return (
        <Dialog>
            <DialogTrigger asChild>
                <Button
                    variant={"outline"}
                    className="group border border-primary/20 h-[150px] items-center justify-center flex flex-col hover:border-primary hover:cursor-pointer border-dashed gap-4"
                >
            ...
            <DialogContent>
                ...
                <DialogFooter>
                    <Button onClick={form.handleSubmit(onSubmit)} disabled={form.formState.isSubmitting} className="w-full mt-4">
                        ...
                        {form.formState.isSubmitting && <ImSpinner2 />}
                    </Button>
                </DialogFooter>
            </DialogContent>
        </Dialog>
